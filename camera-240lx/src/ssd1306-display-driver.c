...
void ssd1306_display_send_command(uint8_t cmd)
{
    ...
    i2c_send_data(&ssd_i2c, 2, cmd_buf);
}
...
void ssd1306_display_init(void)
{
    ...
    printk("in ssd display init function\n");
    // set up the i2c object
    ssd_i2c.bsc = BSC1;
    ssd_i2c.sda = (Pin) { SSD1306_I2C_SDA };
    ssd_i2c.scl = (Pin) { SSD1306_I2C_SCL };
    ssd_i2c.speed_hz = SSD1306_I2C_HZ;
    ssd_i2c.slave_addr = SSD1306_DISPLAY_ADDRESS;
    i2c_init(&ssd_i2c);
...
void ssd1306_display_show(void)
{
    ...
    i2c_send_data(&ssd_i2c, sizeof(i2c_buffer), i2c_buffer);
}
...
DisplayConfig ssd1306_display_get_config(void) {
    DisplayConfig cfg = {
        ...
        .pitch  = SSD1306_DISPLAY_WIDTH,
        .buffer = (uint32_t *)display_buffer,
    };
    return cfg;
}
