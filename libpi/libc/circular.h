...
static inline void cq_init(cq_t *c, unsigned errors_fatal_p) {
    ...
    assert(!cq_full(c));
    assert(cq_nelem(c) == 0);
    // Aditi note: I was getting type errors so I force-casted everything
    // cqe_t e = 0x12;
    // assert(cq_pop_nonblock(c,&e) == 0 && e == 0x12);
    cqe_t e = (cqe_t)(unsigned long)0x12;
    assert(cq_pop_nonblock(c,&e) == 0 && e == (cqe_t)(unsigned long)0x12);
}
